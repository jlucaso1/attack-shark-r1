...
pub const DEVICE_NAME: &str = "Attack Shark R1";
...
const UHID_PHYS_NAME: &[u8] = b"uhid/attack-shark-r1";
const UHID_EVENT_BUFFER_SIZE: usize = 4380;
const UHID_EVENT_MIN_BYTES: usize = 10;
const UHID_NAME_MAX_LEN: usize = 127;
const UHID_REPORT_ID_BATTERY: u8 = 0x01;
const UHID_BUTTON_NONE: u8 = 0x00;
pub const MAX_BATTERY_PERCENTAGE: u8 = 100;

pub const UHID_BATTERY_REPORT_LEN: usize = 4;
const UHID_GET_REPORT_HEADER_LEN: usize = 12;
const UHID_GET_REPORT_REPLY_LEN: usize = UHID_GET_REPORT_HEADER_LEN + UHID_BATTERY_REPORT_LEN;
const UHID_INPUT2_HEADER_LEN: usize = 6;
const UHID_INPUT2_EVENT_LEN: usize = UHID_INPUT2_HEADER_LEN + UHID_BATTERY_REPORT_LEN;

fn encode_battery_report(percentage: u8, is_charging: bool) -> [u8; UHID_BATTERY_REPORT_LEN] {
    [
        UHID_REPORT_ID_BATTERY,
        UHID_BUTTON_NONE,
        percentage.min(MAX_BATTERY_PERCENTAGE),
        is_charging as u8,
    ]
}
...
impl UhidBatteryDevice {
    ...
    pub fn create(
        ...
        let name_len = name_bytes.len().min(UHID_NAME_MAX_LEN);
        create_ev.req.name[..name_len].copy_from_slice(&name_bytes[..name_len]);

        create_ev.req.phys[..UHID_PHYS_NAME.len()].copy_from_slice(UHID_PHYS_NAME);
        ...
        let current_percentage = Arc::new(AtomicU8::new(initial_percentage.min(MAX_BATTERY_PERCENTAGE)));
        ...
        std::thread::spawn(move || {
            let mut buf = [0u8; UHID_EVENT_BUFFER_SIZE];
            while run_clone.load(Ordering::Relaxed) {
                match read_file.read(&mut buf) {
                    Ok(0) => break,
                    Ok(n) if n >= UHID_EVENT_MIN_BYTES => {
                        ...
                        if ev_type == UHID_GET_REPORT {
                            ...
                            let mut reply = UhidGetReportReplyEvent {
                                ...
                                req: UhidGetReportReplyReq {
                                    id: req_id,
                                    err: 0,
                                    size: UHID_BATTERY_REPORT_LEN as u16,
                                    data: [0u8; 4096],
                                },
                            };
                            let payload = encode_battery_report(current_pct, current_chg);
                            reply.req.data[..UHID_BATTERY_REPORT_LEN].copy_from_slice(&payload);

                            let reply_slice = unsafe {
                                std::slice::from_raw_parts(
                                    (&reply as *const UhidGetReportReplyEvent) as *const u8,
                                    UHID_GET_REPORT_REPLY_LEN,
                                )
                            };
    ...
    pub fn update_battery(&mut self, percentage: u8, is_charging: bool) -> Result<(), DriverError> {
        let pct = percentage.min(MAX_BATTERY_PERCENTAGE);
        ...
        let payload = encode_battery_report(pct, is_charging);
        input_ev.req.data[..UHID_BATTERY_REPORT_LEN].copy_from_slice(&payload);

        let raw_slice = unsafe {
            std::slice::from_raw_parts(
                (&input_ev as *const UhidInput2Event) as *const u8,
                UHID_INPUT2_EVENT_LEN,
            )
        };
